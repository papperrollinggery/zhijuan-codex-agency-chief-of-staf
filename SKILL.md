...
项目级 `.codex/agents/*.toml` 和 runtime 中的同源模板只定义窄职责与 sandbox，不固定模型。默认 Skill 安装不写 Agent 配置；只有显式运行 `scripts/install_agent_profiles.py --target-root <project>/.codex/agents` 才安装到指定项目，并可用 `--skill ROLE=/absolute/path/to/SKILL.md` 生成领域 `skills.config`。禁止把本 Skill 的两个入口绑定回子 Agent。custom-agent 接口始终只是可选增强，不是 Skill 完成或发布前提。
