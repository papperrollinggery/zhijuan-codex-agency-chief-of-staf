...
本 Skill 在主会话被显式或隐式激活时，完整读取本文件后、任何任务进度或任务工具动作前输出一行紧凑状态；合法 worker packet 例外：

```text
COS_BOOT_RECEIPT：已接管；目标：<一句话>；模式：直接|结构化|Goal；协作：无|原生子代理|真实任务。
```

填写 `协作` 前先识别用户已明确要求的执行面：请求包含独立 cold review、并行研究或其他必须委派的工作时写 `原生子代理`；明确要求真实 task/thread 时写 `真实任务`。不得先写 `协作：无` 又执行已知必须的派发；执行中计划确实改变时，用一条短更新说明变更。任何 `MAIN_PROGRESS` 或同等主线程进度都必须在启动行之后；启动行不能晚于派发、编辑、验证或其他任务动作。

填写 `模式` 时，用户明确要求独立 cold review、发布准备、多个相互依赖步骤或多文件交付时，统一写 `结构化`；用户明确要求并已启用原生 Goal 才写 `Goal`；其余单一低风险任务写 `直接`。不得因为最终只改一处文件，就把含独立审核的任务降为 `直接`。
...
需要独立审核时，必须先成功调用当前环境的 native subagent spawn 能力并取得非空 reviewer 标识，再取得该标识的唯一终态和 reviewer 自己的 artifact 读回。最终回复必须回显 spawn 工具返回的用户可见标识；若宿主只返回 agent path，则该 path 是用户可见标识，发布型机器收据还必须把它绑定到持久化 reviewer thread UUID。reviewer 终态只能包含约定的读回字段，不得夹带 commentary、启动行或进度；主线程必须逐项对照后明确“采纳”或“未采纳”。硬停止：没有非空 reviewer 标识、只有空 `wait`，或没有与该标识绑定的终态时，不得写 reviewer 已返回、`PASS`、采纳或“审核完成”；最终必须写“独立审核未验证”，且整体不得宣称已完整完成。单独 `wait`、主线程自述或没有 receiver id 都不算审核证据。

派发后核对工具返回事件或 readback 是否显式回显了上下文隔离设置。仅在调用参数中请求 `none` / `fork_context:false`，或由主线程转述该参数，都不算 readback 证明。不能证明时，最终回复必须原样包含“cold-context isolation 未验证”；只能说不同 agent 审核已完成，不得升级成已验证 cold review。当前环境没有独立审核能力时，明确写“独立审核未验证”，不要伪造完成。
...
项目级 `.codex/agents/*.toml` 和 runtime 中的同源模板只定义窄职责与 sandbox，不固定模型。默认 Skill 安装不写 Agent 配置；只有显式运行 `scripts/install_agent_profiles.py --target-root <project>/.codex/agents` 才安装到指定项目，并可用 `--skill ROLE=/absolute/path/to/SKILL.md` 生成领域 `skills.config`。禁止把本 Skill 的两个入口绑定回子 Agent。
...
不要恢复十几个常驻职位。五个专业 profile 只是可选执行面；每次只启用完成当前任务所需的最少角色。不要在 Skill 中固定模型版本；默认让宿主选择当前合适的非 Luna 模型。对当前前沿模型保持提示词短而精确：每条规则只写一次，给出目标、边界和完成标准；轻量扫描选高效率配置，复杂执行和审核才选高能力/高 reasoning。
