...
需要独立审核时，必须先成功调用当前环境的 native subagent spawn 能力并取得非空 reviewer id，再取得该 id 的唯一终态和 reviewer 自己的 artifact 读回。reviewer 终态只能包含约定的读回字段，不得夹带 commentary、启动行或进度；主线程必须逐项对照后明确“采纳”或“未采纳”。硬停止：没有非空 reviewer id、只有空 `wait`，或没有与该 id 绑定的终态时，不得写 reviewer 已返回、`PASS`、采纳或“审核完成”；最终必须写“独立审核未验证”，且整体不得宣称已完整完成。单独 `wait`、主线程自述或没有 receiver id 都不算审核证据。
