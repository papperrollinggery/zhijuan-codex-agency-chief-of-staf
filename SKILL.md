...
重要边界：Skill 描述和 `allow_implicit_invocation: true` 只能提高本 Skill 被选择的概率，不能强制所有复杂任务默认进入幕僚长流程。需要项目默认路由时，必须把 `references/AGENTS_ROUTING_SNIPPET.md` 合入项目级 AGENTS.md；没有项目级 AGENTS.md 路由时，黑盒复杂任务只能作为前测信号，不能被包装成启动保证。
...
3. 用户没有要求线程且任务为 T0/T1 时，可以 `thread_dispatch_decision: no_dispatch`，但必须写明原因。
