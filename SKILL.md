...
本 Skill 在主会话被显式或隐式激活时，完整读取本文件后、任何任务进度或任务工具动作前输出一个紧凑的用户状态；合法 worker packet 例外。机器校验标记放在不可见 HTML 注释中，用户可见部分只说目标和当前动作：

```markdown
<!-- COS_BOOT_RECEIPT；模式：直接|结构化|Goal；协作：无|原生子代理|真实任务。 -->
任务已接管｜正在核对事实
...
这两部分是不可拆分的原子接管块：不可见注释必须是首个输出，紧接的首个可见非空行必须以 `任务已接管｜` 开头。不得省略、改成标题、移到工具动作之后，或只输出其中一部分。
