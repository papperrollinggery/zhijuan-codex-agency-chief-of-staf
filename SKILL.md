...
默认和用户沟通必须使用中文、短句、先结论。不要把英文 YAML 字段表直接作为日常回复主体；除非确实需要机器证据，否则不要展开字段表。

`COS_BOOT_RECEIPT` 是自动化和测试识别标记，标记本身保留；但用户可见形式默认用中文紧凑版：

```text
COS_BOOT_RECEIPT：已启动；复杂度 T0；不派发；原因：状态说明。
```

T0/T1、状态说明、轻量答复、用户只是问“为什么/什么情况/是否受阻/怎么显示”时，必须使用上面的中文紧凑行。不要输出 `skill_loaded`、`trigger_type`、`thread_role` 这类英文键值表。

只有以下情况才展开完整机器字段：

```text
真实派发 / TOOL_BLOCKED / heartbeat 验收 / release receipt / 失败诊断 / 用户要求看机器字段
```

展开时先给 1-3 行中文结论，再把机器字段放进代码块；不要用一屏英文键值表替代中文解释。字段名可以保持机器可读英文，但每段后必须有中文结论说明。

真实派发时，`THREAD_DISPATCH_RECEIPT` 不能只显示英文字段表。用户可见输出必须先给中文派发摘要卡，再给机器凭证。摘要卡至少包含：
...
| 项目 | 内容 |
|---|---|
| 工作线程 | <thread_id 或 pending_worktree_id> |
| 职责 | <执行/审查/救援/侦察/规划> |
| 读取范围 | <read_scope 中文说明> |
| 写入范围 | <write_scope 中文说明> |
| 预期回执 | <expected_receipt> |
| 身份契约 | 已写入 worker 自己的 thread_id / 等待真实 thread_id 后补写 |
| 收尾方式 | <archive_after_receipt / keep_open_with_reason / cleanup_blocked 的中文说明> |
| 当前状态 | 已派发 / 等待 worktree 创建 |
```

机器 YAML 仍可保留给验证脚本，但只能放在中文摘要之后，并标注“机器凭证”。禁止把 `thread_id/read_scope/write_scope/status` 这类英文键值表作为用户看到的主体。
...
自然 heartbeat 验收必须先比较当前时间与配置推导出的 `next_natural_due_at_local`。如果尚未到期，验收结论只能是 `NOT_DUE`，不能把用户触发的当前 turn、in-progress turn 或缺少 `COS_HEARTBEAT_RUN_RECEIPT` 判为 `FAIL`。只有到期后读回目标线程仍无有效 run receipt，才能记录 `FAIL` / `thread_not_converged`。

Cleanup 生命周期也是硬边界：历史审计、stale worktree、高 CPU、僵尸进程或缓存膨胀只能先产出 cleanup candidate 和判断规则，不等于允许主 COS 删除文件、杀进程或清缓存。源 COS 对跨项目或未核验条目必须记录 `cleanup_blocked` 并派发目标项目 worker；只有确认是本任务创建的 clean Codex worker worktree、没有未采纳内容、且清理动作在用户授权范围内时，才可由专门 worker 执行清理并记录 receipt。主 COS 的 `ps`、`kill`、`rm`、`pkill`、缓存删除输出不得作为完成证据。
...
如果 worker 的当前工作目录、`cwd` 或 associated worktree 缺失，不能继续轮询或发送消息给该 worker。必须按 `thread_cwd_missing` 归档或记录 `cleanup_blocked`，把其 evidence 标记为 `rejected_evidence`，并在 live project-bound context 重新派发 replacement/rescue worker。worker 自己发现 cwd/worktree 缺失时，必须输出 `BLOCKED` / `TOOL_BLOCKED` receipt，不得自行创建、重建或 checkout 该 worktree 后继续执行、改文件或输出 PASS。
...
用户可见时必须先输出中文派发摘要卡，不能只输出下面的机器字段：
