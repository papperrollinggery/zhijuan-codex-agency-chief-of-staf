...
- Missing worker cwd/worktree is now a hard failure mode: `thread_cwd_missing` must be recorded with `thread_not_converged`, `adoption_status: rejected_evidence`, and `cleanup_status: archived | cleanup_blocked`; historical audit and activation-contract fixtures now cover the case.
