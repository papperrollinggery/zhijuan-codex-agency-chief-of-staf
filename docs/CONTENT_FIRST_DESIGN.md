...
Reserved worker and execution-session markers fail closed. Native `create_thread` may transport an Execution Session inside an exact `<codex_delegation>` envelope; the Runtime unwraps only that host form, rejects Worker/malformed transports, and requires the source to be a user-owned Root in both App Server and canonical state before binding. Prompts that predate the required explicit Canonical Skill field cannot be rebound: unbound tasks must regenerate the launch packet, and any already-launched old Native task must be closed or recorded as a cleanup blocker before replacement. Only an already-bound current-packet session whose stored readback lacks transport fields may gain those fields after a fresh mechanical recheck. Execution Root has orchestration depth zero; its terminal workers cannot invoke this Skill or spawn another layer. Root execution-model resolution remains separate from Subagent cost routing and cannot silently downgrade a requested effort.
