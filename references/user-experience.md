...
Direct Mode 仍可使用“任务已接管｜正在核对事实”。前台默认只显示当前阶段、当前结果、下一项动作，以及真正需要用户决定的唯一问题。Profile、exact model ID、receipt、Thread schema、hash 和 packet 留在后台；用户明确要求技术证据时再展示。
...
先写本阶段结论，再列最多三项有意义的状态。优先写“已找到根因”“正在制作交互稿”“验证发现一个阻塞”，不要写工具名、命令、回值或内部阶段代码。进度由工作项开始/完成、产物、验证、阻塞、Team Plan、Review、任务完成和归档事件驱动；没有新信息时不要按固定时间重复同义状态。
