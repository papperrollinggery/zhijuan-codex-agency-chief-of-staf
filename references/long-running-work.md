...
- 目标仍模糊时先补齐真正影响结果的缺口；只有用户明确要求保存计划时才创建 `.agency` 执行清单。已授权“分析并完成”的工作不在讨论后重复确认。
- 多轮持续推进保持当前目标；只有用户明确要求 Goal 才使用原生 Goal。若同时有 `.agency` Task，Goal 管长期停止条件，Task 记录 Work Item、进度、证据和归档。
- 单轮可完成：不用 Goal。
