...
在公开发布、安全、高风险迁移、结构性跨模块变更、复杂客户成果或用户明确要求独立审核时读取。普通低风险多文件修改和客户文稿微调不自动需要 Cold Review；Assured 也不自动启用模型身份、receipt 或发布门禁。
...
- `已验证`：本轮有直接证据。
- `部分验证`：已完成部分检查，明确列出尚未覆盖的完成标准。
- `未验证`：尚未运行、工具不可用或只有历史线索。
- `验证失败`：本轮验证已运行且失败。
- `TOOL_BLOCKED`：所需执行能力不可用，说明已完成部分和具体缺口。
