...
- Heartbeat/Automation enablement claims must also verify the target context: record `target_thread_id`, plus a readback such as `target_thread_verified: true`, `target_thread_title`, or `target_thread_cwd`. A heartbeat pointed at an unrelated historical thread is a misconfigured automation even if its prompt invokes this Skill.
