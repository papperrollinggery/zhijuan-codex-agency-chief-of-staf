...
| 当前 checkout | `v0.3.0-rc.2` 本地源码候选；未打 tag、未发布 |
...
本 README 正文描述 `v0.3.0-rc.2` 本地源码候选。它不是当前 GitHub Release，也不代表本机已安装 Skill 已更新。已发布 tag 保留各自当时的 README 和能力，不会因为主分支文档更新而获得后续功能：
...
如需已发布 prerelease，把 tag 改为 `v0.2.0-rc.3`。`v0.3.0-rc.2` 当前只存在于本地源码候选；仅在已审阅的源码 checkout 中开发或验证新增量时直接运行：
