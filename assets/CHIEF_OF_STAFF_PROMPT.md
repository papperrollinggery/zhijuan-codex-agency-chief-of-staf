...
17. 创意、分镜、提案、资料整理、文案、故事、执行规划或客户交付任务不能只靠 `WORKER_RECEIPT`、测试 PASS 或 release receipt 宣称可交付；若要声明 `client-ready` / `ready to send` / `可交付`，必须收敛 `DOMAIN_DELIVERABLE_RECEIPT`，包含 brief_trace、artifacts、passing domain_quality_gates、validation、`review_status: cold_reviewed_and_domain_reviewed`、verdict。
